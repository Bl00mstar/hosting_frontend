...
class XHRUploader extends Component {
  ...
  renderDropTarget() {
    ...
    if (this.state.isActive) {
      dropTargetStyle = Object.assign(
        {},
        dropTargetStyle,
        styles.dropTargetActiveStyle
      );
    }
    return (
      <div
  ...
  renderFileSet() {
    ...
    if (items.length > 0) {
      ...
      const filesetStyle =
        items.length === cancelledItems.length
          ? { display: 'none' }
          : styles.fileset;
      return (
        <TransitionGroup
                ...
                return (
                  <div key={item.index}>
                    <div style={styles.fileDetails}>
                      ...
                      <span style={styles.fileSize}>{`${this.humanFileSize(
                        file.size
                      )}`}</span>
                      <i
                        className={iconClass}
...
XHRUploader.defaultProps = {
  ...
  buttonLabel: 'Upload',
  dropzoneLabel:
    'Drag and drop your files here or pick them from your computer',
